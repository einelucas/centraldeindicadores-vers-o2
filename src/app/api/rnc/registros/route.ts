...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (body.tempoTratativa !== undefined)
      data.tempoTratativa = body.tempoTratativa;

    const noChange =
      (body.status === undefined || body.status === existing.statusRnc) &&
      (body.tempoTratativa === undefined ||
        body.tempoTratativa === existing.tempoTratativa);
    if (noChange) {
...
/** DELETE /api/rnc/registros — exclui registros por IDs (só ADMIN). */
