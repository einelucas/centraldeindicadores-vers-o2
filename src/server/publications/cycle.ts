 ...
 * `cycleFromYearSemester(ano, semestre)` para o ano de início do próprio
 * período — nunca com um range parcial/arbitrário. Usado para decidir se uma
 * publicação pode ser identificada por Ano+Semestre (colunas `cycleYear`/
 * `cycleSemester` de `IndicatorPublication`) ou se fica só no `payload`
 * (publicações "sem restrição", sem período, ou com um recorte que não é um
 * semestre inteiro).
 *
...
export function resolvePublicationCycle(period: PeriodRange | null): PublicationCycle | null {
  ...
  const candidates: PublicationCycle[] = [
    ...
    { year: normalized.startYear, semester: "S1" },
  ];
